...
int main( int argc, char **argv ) {
  ...
  if ( mpi_rank == mpi_root ) {
    std::cout << "MCNLA "
              << MCNLA_MAJOR_VERSION << "."
              << MCNLA_MINOR_VERSION << "."
              << MCNLA_PATCH_VERSION << " Naive KN demo" << std::endl << std::endl;
  }
  ...
  ValType     tol       = ( argc > ++argi ) ? atof(argv[argi]) : -1;
  ...
  StatisticsSet set_time(num_test), set_time1(num_test), set_time2(num_test), set_time2c(num_test), set_time3(num_test);

  // ====================================================================================================================== //
  // Initialize driver
  mcnla::isvd::Solver<ValType,
  ...
  // ====================================================================================================================== //
  // Generate matrix
  mcnla::matrix::DenseDiagonalMatrix<ValType> matrix_a(m);
  ...
  for ( int t = -skip_test; t < num_test; ++t ) {
    ...
    if ( mpi_rank == mpi_root  ) {
      ...
      auto time2c = integrator.time2c();
      auto time3 = integrator.time3();
      std::cout << std::setw(log10(num_test)+1) << t
                << " | time: " << time << " (" << time1 << " / " << time2
                               << "(" << time2c << ") / " << time3 << ")" << std::endl;
      if ( t >= 0 ) {
        set_time(time); set_time1(time1); set_time2(time2); set_time2c(time2c); set_time3(time3);
      }
    }
  }
  ...
  if ( mpi_rank == mpi_root ) {
    ...
    std::cout << std::endl;
    std::cout << "Average computing time of integrating Q:     " << set_time2.mean() - set_time2c.mean()
                                                                                      << " seconds." << std::endl;
    std::cout << "Average communication time of integrating Q: " << set_time2c.mean() << " seconds." << std::endl;
    std::cout << std::endl;
  }
