...
TEST(NaiveKolmogorovNagumoIntegratorTest, Test) {
  ...
  // Initializes
  mcnla::isvd::NaiveKolmogorovNagumoIntegrator<mcnla::matrix::DenseMatrix<ScalarType>>
      integrator(parameters, MPI_COMM_WORLD, 0);
  integrator.initialize();
  parameters.initialized_ = true;

  // Copies data
  for ( auto i = 0; i < Nj; i++ ) {
    mcnla::blas::copy(set_q_true.getPage(mpi_rank*Nj + i), integrator.getSetQ().getPage(i));
  }
