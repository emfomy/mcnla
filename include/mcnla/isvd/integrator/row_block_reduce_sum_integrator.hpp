...
void MCNLA_TMP::initializeImpl() noexcept {
  ...
  collection_b_.reconstruct(dim_sketch, dim_sketch_total, (num_sketch+1)/2);

  matrix_w_.reconstruct(dim_sketch, dim_sketch);
  matrix_tt_.reconstruct(dim_sketch, dim_sketch);
...
///
template <typename _Val>
RowBlockReduceSumIntegrator<_Val>& MCNLA_TMP::setMaxIteration(
    const index_t max_iteration
) noexcept {
...
///
template <typename _Val>
RowBlockReduceSumIntegrator<_Val>& MCNLA_TMP::setTolerance(
    const RealValT<_Val> tolerance
) noexcept {
