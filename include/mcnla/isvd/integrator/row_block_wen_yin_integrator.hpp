...
void MCNLA_TMP::runImpl(
  ...
  _Val taug = tau0_, zeta = 1.0, phi = one_2n * la::dot(matrix_bc.vec());

  // mu := tr( Dgc ) - norm( Dc )_F
  _Val mu = la::asum(matrix_dgc_.diag().vec()) - la::dot(matrix_dc_.vec());
  ...
  for ( iteration_ = 0; iteration_ < max_iteration_; ++iteration_ ) {
    ...
    // Dg+ [in Ggc] := 1/N * B+' * Bg+ [in Bgc]
    la::mm(matrix_bp.t(), matrix_bgc_, matrix_dgc_, one_n);

    // mu := tr( Dg+ [in Ggc] ) - norm( D+ [in Dc] )_F
...
///
template <typename _Val>
RowBlockWenYinIntegrator<_Val>& MCNLA_TMP::setMaxIteration(
    const index_t max_iteration
) noexcept {
...
///
template <typename _Val>
RowBlockWenYinIntegrator<_Val>& MCNLA_TMP::setTolerance(
    const RealValT<_Val> tolerance
) noexcept {
