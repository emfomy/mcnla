...
std::ostream& operator<< ( std::ostream &out, const DenseVectorIteratorBase<__Scalar, __Vector> &iterator ) {
  ...
  return out << "(" << std::setw(width) << iterator.getIdx() << ")  " << std::setw(ios_width) << iterator.getValuePtr();
}
...
_Scalar& DenseVectorIteratorBase<_Scalar, _Vector>::getValuePtr() const noexcept {
