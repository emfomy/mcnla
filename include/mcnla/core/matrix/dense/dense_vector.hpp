...
DenseVectorIterator<_Val> GeVecS<CpuTag, DenseTag, _Val>::find(
    const index_t idx
) noexcept {
...
///
template <typename _Val>
DenseVectorConstIterator<_Val> GeVecS<CpuTag, DenseTag, _Val>::find(
    const index_t idx
) const noexcept {
  mcnla_assert_gelt(idx, 0, this->len());
...
///
template <typename _Val>
DenseVectorConstIterator<_Val> GeVecS<CpuTag, DenseTag, _Val>::cfind(
    const index_t idx
) const noexcept {
  return find(idx);
}
