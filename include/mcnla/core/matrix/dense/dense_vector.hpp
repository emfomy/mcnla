...
DenseVectorIterator<_Val> GeVecI<CpuTag, DenseTag, _Val>::find(
    const index_t idx
) noexcept {
...
///
template <typename _Val>
DenseVectorConstIterator<_Val> GeVecI<CpuTag, DenseTag, _Val>::find(
    const index_t idx
) const noexcept {
  mcnla_assert_gelt(idx, 0, this->length());
...
///
template <typename _Val>
DenseVectorConstIterator<_Val> GeVecI<CpuTag, DenseTag, _Val>::cfind(
    const index_t idx
) const noexcept {
  return find(idx);
}
